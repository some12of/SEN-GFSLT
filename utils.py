...
def init_distributed_mode(args):#分布式训练初始化
...
def data_augmentation(resize=(320, 240), crop_size=224, is_train=True): #输入参数resize、crop_size和is_train返回一个裁剪矩形及其大小
    if is_train:
        left, top = np.random.randint(0, resize[0] - crop_size), np.random.randint(0, resize[1] - crop_size) #left和top分别是裁剪矩形的左边界和上边界
    else:
        left, top = (resize[0] - crop_size) // 2, (resize[1] - crop_size) // 2 #通过(resize[0] - crop_size) // 2和(resize[1] - crop_size) // 2计算得到，确保裁剪矩形居中

    return (left, top, left + crop_size, top + crop_size), resize #函数可能会返回((10, 20, 214, 240), (320, 240))，表示将图像裁剪为[10, 20]垂直范围、[214, 240]水平范围和[320, 240]大小。
...
class KLLoss(torch.nn.Module):
    ...
    def forward(self, prediction, label):
        ...
        # prediction = prediction * F.softmax(prediction/1000, dim=0)*len(prediction)#相应的DSL来修正预测的相似度评分
