...
class GraphEmbv(nn.Module):
    ...
    def __init__(self, embed_dim, sim_dim):
        ...
        """ VisualSA starting
        """
        self.conv3=nn.Conv1d(1024, 1024, kernel_size=3, stride=1, padding='same')
        self.emb_local_li = nn.Linear(embed_dim, embed_dim)
        # self.emb_local_bn = nn.BatchNorm1d(36)
        self.emb_local_bn =MaskedNorm(num_features=1024, norm_type='batch')
        self.tanh = nn.Tanh()
        self.dropout = nn.Dropout(0.4)
    ...
    def forward(self, img_emb, cap_emb, cap_lens, mask):
        
        n_image = img_emb.size(0)
        n_caption = cap_emb.size(0)
        n_image_length=img_emb.size(1)
        """ get global image embedding
        """
        # get enhanced global images by self-attention
        img_ave = torch.mean(img_emb, 1)
        l_emb = self.dropout(self.tanh(self.emb_local_bn(self.emb_local_li(img_emb),mask.cuda())))
        ...
        new_global = self.conv3(img_emb.permute(0,2,1)).permute(0,2,1)
        norm_glo = norm_glo.unsqueeze(1).repeat(1,n_image_length,1)
        ...
        for i in range(n_caption):
            ...
            cap_glo4par_i = cap_glo_i.repeat(n_image_length,1).unsqueeze(0)
            ...
            sim_glo_expand = sim_glo.unsqueeze(1).repeat(1,n_image_length,1)
            similarity = sim_par_v.mul(sim_glo_expand)
            weights = self.emb_similarity(similarity).squeeze(2)
            weights = self.sigmoid(weights).unsqueeze(2)
            ...
            weights_list.append(sim_loc_v)
        weights_list=torch.stack([tensor for tensor in weights_list], dim=0)
        return weights_list
