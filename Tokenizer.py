...
def shift_tokens_right(input_ids: torch.Tensor, pad_token_id: int, ignore_index: int = -100):#输入序列向右移动一个位置，并在序列的开始位置放置一个适当的开始标记。
    """
    Shift input ids one token to the right, and wrap the last non pad token (the <LID> token) Note that MBart does not
    have a single `decoder_start_token_id` in contrast to other Bart-like models.
    """
    prev_output_tokens = input_ids.clone() #克隆输入tensor input_ids 到 prev_output_tokens

    assert pad_token_id is not None, "self.model.config.pad_token_id has to be defined."
    # replace possible -100 values in labels by `pad_token_id`
    prev_output_tokens.masked_fill_(prev_output_tokens == -100, pad_token_id) #所有等于-100的token替换为pad_token_id。
    index_of_eos = (prev_output_tokens.ne(pad_token_id).sum(dim=1) - 1).unsqueeze(-1)#计算每个序列中的最后一个非padding token的位置，这个位置就是EOS token应该放置的地方。
    for ii, ind in enumerate(index_of_eos.squeeze(-1)):
        input_ids[ii, ind:] = ignore_index #根据计算出的位置，将每个序列的EOS token之后的所有token设置为ignore_index。
    decoder_start_tokens = prev_output_tokens.gather(1, index_of_eos).squeeze() #通过gather函数，将最后一个非padding token收集到一个新的tensor
    prev_output_tokens[:, 1:] = prev_output_tokens[:, :-1].clone() #将prev_output_tokenstensor向右移动一个位置，并将第一个token设置为decoder_start_tokens。
...
class GlossTokenizer_G2T(BaseGlossTokenizer):#用于将一批手语序列转换为模型可以接受的输入格式
    def __init__(self, tokenizer_cfg):
        super().__init__(tokenizer_cfg)
        self.src_lang = tokenizer_cfg['src_lang']

    def __call__(self, batch_gls_seq):#接受一个手语序列的批次（batch_gls_seq）作为输入
        # batch
        max_length = max([len(gls_seq.split()) for gls_seq in batch_gls_seq]) + 2  # include </s> <lang>，计算批次中最长的手势序列的长度，加上2（可能是用于特殊符号）。
        batch_gls_ids = [] #初始化一个空列表，用于存储批次中每个手势序列的ID。
        attention_mask = torch.zeros([len(batch_gls_seq), max_length], dtype=torch.long) #初始化一个注意力掩码张量，用于在解码过程中忽略填充的<pad>符号。大小为批次大小×最大长度
        for ii, gls_seq in enumerate(batch_gls_seq):#遍历批次中的每一个手语序列。
            gls_ids = [self.gloss2id[gls.lower() if self.lower_case else gls] for gls in gls_seq.split()] #将手语序列转换为ID列表，考虑是否需要小写化。
            # add </s> <lang> + padding
            gls_ids = gls_ids + [self.gloss2id['</s>'], self.gloss2id[self.src_lang]] #在序列末尾添加结束符号<eos>和源语言符号。
            attention_mask[ii, :len(gls_ids)] = 1 #将注意力掩码张量的对应位置设置为1，表示这些位置是有效的输入。
            gls_ids = gls_ids + (max_length - len(gls_ids)) * [self.gloss2id['<pad>']] #将序列补全到最大长度，填充符号为<pad>。
            batch_gls_ids.append(gls_ids) #将当前序列的ID列表添加到批次中。
        input_ids = torch.tensor(batch_gls_ids, dtype=torch.long) #将批次列表转换为一个PyTorch张量
        attention_mask = torch.tensor(attention_mask, dtype=torch.long) #将注意力掩码张量转换为一个PyTorch张量
        return {'input_ids': input_ids, 'attention_mask': attention_mask} #返回一个包含“input_ids”和“attention_mask”的字典。
