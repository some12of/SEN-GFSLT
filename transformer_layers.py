...
class DeformableMultiHeadedAttention(nn.Module):
    ...
    def forward(self, k: Tensor, v: Tensor, q: Tensor, mask: Tensor = None):
        ...
        #根据k生成基于相似度量的高斯注意权重
        # 计算余弦相似度
        norm_k = F.normalize(smaple_k, p=2, dim=-1)
        # norm_q = F.normalize(q, p=2, dim=-1)
        # matmul进行矩阵乘法，计算点积  
        similarity_matrix = torch.matmul(norm_k, norm_k.transpose(3, 4))  
        tensor=similarity_matrix.reshape(-1,self.num_keys,self.num_keys)
        # 对4x4的相似度矩阵按行相加，得到4x1的张量（对于每个样本）  
        row_sums = torch.sum(tensor, dim=-1, keepdim=True)
        # 找到每个样本中最大值的位置，即锚点的位置  
        anchor_indices = torch.argmax(row_sums, dim=1)  # dim=1表示沿着行（第二个维度）找到最大值的位置  
        # 初始化权重张量，形状与输入张量的最后一个两个维度相同，但样本数量不变，即(num_samples, 4, 4)  
        weights = torch.zeros_like(row_sums[:,:,0])  
        # 设定标准差，控制高斯分布的宽度  
        sigma = 1.0  
        # 为每个样本生成高斯分布权重  
        for i in range(tensor.size(0)):  
            anchor_pos = anchor_indices[i].item()  # 获取当前样本的锚点位置，并使用.item()将其转换为Python标量  
            mean = torch.tensor([anchor_pos], dtype=torch.float32)  # 均值位置就是锚点位置，需要转换为张量以便广播  
        
            # 生成一维高斯分布权重，并限制在[0, 1]范围内  
            # 这里我们只需要为锚点所在行生成权重，因此只生成一个长度为4的一维张量  
            # 注意：我们需要为高斯分布生成一个与行长度相同的x值数组  
            # x_values = torch.arange(4, dtype=torch.float32)  
            # gaussian_weights_1d = norm.pdf(x_values, mean, sigma)  
            # gaussian_weights_1d = torch.clamp(gaussian_weights_1d, 0, 1)  # 使用torch.clamp来限制值  
        
            # 生成一维高斯分布权重，并限制在[0, 1]范围内  
            # 使用数学公式直接在PyTorch中计算高斯分布  
            mean_tensor = mean.expand(self.num_keys)  # 扩展mean张量以匹配x_values的形状  
            x_values = torch.arange(self.num_keys, dtype=torch.float32)  
            gaussian_weights_1d = torch.exp(-0.5 * ((x_values - mean_tensor) / sigma) ** 2) / (sigma * torch.sqrt(torch.tensor(2 * torch.pi)))  
            gaussian_weights_1d = gaussian_weights_1d / gaussian_weights_1d.sum()  # 归一化，确保权重之和为1（可选）

            # 将一维高斯分布权重设置到对应样本的锚点所在行上  
            # 注意：我们需要扩展gaussian_weights_1d的维度以匹配weights张量的维度  
            weights[i, :] = gaussian_weights_1d   
        weights=weights.reshape(batch_size, num_heads,-1,self.num_keys).cuda()
        weights = self.softmax(weights)
        weights=weights*attention
        alpha = 0.99
        attention=alpha*attention+(1-alpha)*weights
        # get context vector (select values with attention) and reshape
        # back to [B, M, D]
